...
if ( ! class_exists( 'Bsmaps_Elementor_Gmap' ) ) {
	/**
	 * Bs_Map shortcode class
	 *
	 * @return void
	 */
	class Bsmaps_Elementor_Gmap extends Widget_Base {
		/**
		 * Slug
		 *
		 * @var string
		 */
		protected $slug = 'bsmaps-elementor-gmap';
		 ...
		 * @var string
		 */
		protected $handler = 'bsmaps-vc-gmap';
		...
		public function get_title() {
			return esc_html__( 'BS Google Map', 'bsmaps' );
		}
		...
		protected function get_general_options() {
			...
			$bsmaps_map_key = get_option( 'bsmaps_gmap_key' );
			if ( ! isset( $bsmaps_map_key ) || '' === $bsmaps_map_key ) {
				$this->add_control(
					'notice',
					array(
						'type' => Controls_Manager::RAW_HTML,
						'raw'  => '<div class="bsmaps-notice">
                                <a target="_blank" href="' . admin_url( 'admin.php?page=bsmaps' ) . '">' . esc_html__( 'Click Here', 'bsmaps' ) . '</a>' . esc_html__( 'to add google map api key.', 'bsmaps' ) . '</div>',
					)
				);
			}
		...
		protected function render() {
			...
			echo do_shortcode( '[bsmaps-vc-gmap-shortcode ' . $arg_strings . ']' );
		}
	}

	\Elementor\Plugin::instance()->widgets_manager->register( new Bsmaps_Elementor_Gmap() );
